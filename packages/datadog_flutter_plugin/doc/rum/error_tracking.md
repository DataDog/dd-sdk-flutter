...
If your application suffers a fatal crash, once your application restarts, the Datadog SDK uploads a crash report to Datadog. For non-fatal errors, the Datadog SDK will upload these errors with other RUM data. 
...
All crash reports that are built with the `--split-debug-info` option set and / or with the `--obfuscate` option set will need to upload their Android Proguard mapping file, and the Dary symbol files generated by the Flutter build process.
...
For a full list of options, refer to the `datadog-ci` [flutter-symbols documentation][6]
