...
void main() {
  ...
  setUp(() async {
    ...
    when(
      () => mockPlatform.createLogger(any(), any()),
    ).thenAnswer((_) => Future.value());
    when(
      () => mockPlatform.destroyLogger(any()),
    ).thenAnswer((_) => Future.value());
    when(
      () => mockPlatform.addGlobalAttribute(any(), any()),
    ).thenAnswer((_) => Future.value());
    when(
      () => mockPlatform.removeGlobalAttribute(any()),
    ).thenAnswer((_) => Future.value());
  ...
  group('basic logger tests', () {
    ...
    test('addAttribute argumentError sent to logger', () async {
      when(
        () => mockPlatform.addAttribute(any(), any(), any()),
  ...
  group('configuration tests', () {
    test('sampleRate is clamped to 0..100', () {
      final lowConfiguration = DatadogLoggerConfiguration(
        remoteSampleRate: -12.2,
      );

      final highConfiguration = DatadogLoggerConfiguration(
        remoteSampleRate: 123.5,
      );
  ...
  group('threshold tests', () {
    ...
    test(
      'threshold set to middle sends call proper platform methods',
      () async {
        final config = DatadogLoggerConfiguration(
        ...
        for (var level in [LogLevel.debug, LogLevel.info]) {
          verifyNever(
            () => mockPlatform.log(
              any(),
              level,
        ...
        for (var level in [LogLevel.warning, LogLevel.error]) {
          verify(
            () =>
                mockPlatform.log(any(), level, any(), null, null, null, any()),
          );
  ...
  if (kDebugMode) {
    group('console printing', () {
      ...
      test(
        'simpleConsolePrint logs to print by default',
        overridePrint(() {
          final config = DatadogLoggerConfiguration(
            remoteLogThreshold: LogLevel.warning,
          );
      ...
      test(
        'simpleConsolePrintForLevel logs to print by default',
        overridePrint(() async {
          final config = DatadogLoggerConfiguration(
            remoteLogThreshold: LogLevel.warning,
            customConsoleLogFunction: simpleConsolePrintForLevel(
              LogLevel.debug,
            ),
          );
          final logger = ddLogs.createLogger(config);
      ...
      test(
        'simpleConsolePrintForLevel filters to correct level',
        overridePrint(() {
          final config = DatadogLoggerConfiguration(
            remoteLogThreshold: LogLevel.warning,
            customConsoleLogFunction: simpleConsolePrintForLevel(
          ...
          expect(printLog, ['[warning] Warn message', '[error] Error message']);
        }),
      );
    });
  }
