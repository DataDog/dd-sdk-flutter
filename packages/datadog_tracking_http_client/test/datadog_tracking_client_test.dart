...
void main() {
  ...
  group('when rum is disabled', () {
    ...
    test('tracking client passes through send calls', () async {
      ...
      final captured =
          verify(() => mockClient.send(captureAny())).captured[0]
              as http.BaseRequest;
  ...
  group('when rum is enabled', () {
    ...
    test('calls stopResource on completion', () async {
      ...
      final future = client.get(
        testUri,
        headers: {'x-datadog-header': 'header'},
      );
    ...
    test(
      'reports streamed response size when contentLength is null (chunked)',
      () async {
        final client = DatadogClient(
          datadogSdk: mockDatadog,
          innerClient: mockClient,
        ...
        final chunkedStream = http.ByteStream(
          Stream.fromIterable([
            [1, 2, 3],
            [4, 5],
          ]),
    ...
    test(
      'send throwingError rethrows and calls stopResourceWithErrorInfo',
      () async {
        final client = DatadogClient(
          datadogSdk: mockDatadog,
          innerClient: mockClient,
    ...
    test(
      ...
      () async {
        final client = DatadogClient(
          datadogSdk: mockDatadog,
          innerClient: mockClient,
          ignoreUrlPatterns: [RegExp('ignore_me.com/a/b')],
        );
    ...
    test(
      'ignoreUrlPatterns performs tracking when urls do not match',
      () async {
        final client = DatadogClient(
          datadogSdk: mockDatadog,
          innerClient: mockClient,
          ignoreUrlPatterns: [RegExp('test_url/my_endpoint')],
        );
        final testUri = Uri.parse('https://test_url/test');
    ...
    test('different tracing headers are same trace id', () async {
      ...
      var datadogTraceInt = BigInt.tryParse(
        captured.headers['x-datadog-trace-id']!,
      );
      ...
      final datadogSpanInt = BigInt.tryParse(
        captured.headers['x-datadog-parent-id']!,
      );
