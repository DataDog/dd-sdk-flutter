...
void main() {
  ...
  testWidgets(
      'captures Icon as ResourceImageNode and saves bytes once for two identical icons',
      (tester) async {
    const iconSize = 32.0;
    ...
    expect(
        capture1!.viewTreeSnapshot.nodes.every((n) => n is ResourceImageNode),
        isTrue);
    final key1 =
        (capture1!.viewTreeSnapshot.nodes[0] as ResourceImageNode).resourceKey;
    final key2 =
        (capture1!.viewTreeSnapshot.nodes[1] as ResourceImageNode).resourceKey;
    expect(key1, key2);
    ...
    expect(
        (capture2!.viewTreeSnapshot.nodes[0] as ResourceImageNode).resourceKey,
        key1);
    expect(
        (capture2!.viewTreeSnapshot.nodes[1] as ResourceImageNode).resourceKey,
        key1);
  ...
  testWidgets('different icon color triggers second saveImageForProcessing',
      (tester) async {
    const iconSize = 28.0;
    final tree = MaterialApp(
      home: SimpleTestCapture(
        ...
        child: Stack(
          children: [
            Positioned(
              ...
              height: iconSize,
              child:
                  const Icon(Icons.star, color: Colors.amber, size: iconSize),
            ),
            Positioned(
              top: 10,
              left: 60,
              ...
              height: iconSize,
              child:
                  const Icon(Icons.star, color: Colors.green, size: iconSize),
            ),
          ],
        ),
      ),
  ...
  testWidgets(
          ...
          children: [
            Positioned(
              ...
              child: const Icon(Icons.favorite,
                  color: Colors.red, size: largeSize),
            ),
            Positioned(
              top: 10,
              left: 60,
              ...
              height: smallSize,
              child: const Icon(Icons.favorite,
                  color: Colors.red, size: smallSize),
            ),
          ],
        ),
      ),
    ...
    expect(capture!.viewTreeSnapshot.nodes.every((n) => n is ResourceImageNode),
        isTrue);
    final key1 =
        (capture!.viewTreeSnapshot.nodes[0] as ResourceImageNode).resourceKey;
    final key2 =
        (capture!.viewTreeSnapshot.nodes[1] as ResourceImageNode).resourceKey;
    expect(key1, key2);
  ...
  testWidgets('maskAll shows Icon placeholder and does not save image',
      (tester) async {
  ...
  testWidgets('maskNonAssetsOnly still captures Material Icon', (tester) async {
    ...
    verify(() => platform.saveImageForProcessing(any(), any(), any(), any()))
        .called(1);
  });

  testWidgets(
      'Icon subtree is ignored so inner RichText is not captured as text',
      (tester) async {
    final tree = MaterialApp(
      home: SimpleTestCapture(
        key: const Key('key'),
